...
resource "aws_ec2_host" "mac" {
  ...
  instance_type     = var.machines[count.index].use_intel ? "mac1.metal" : var.machines[count.index].use_m2_pro ? "mac2-m2pro.metal" : "mac2.metal"
  ...
  tags = {
    ...
    region             = var.aws_region,
    environment        = var.aws_tag_environment,
    name               = "${substr(var.aws_region, 0, 2)}-${var.machines[count.index].name}.${count.index}",
    dataClassification = var.aws_tag_dataClassification,
    iac                = var.aws_tag_iac,
    pii                = var.aws_tag_pii,
    description        = var.aws_tag_description,
...
resource "aws_instance" "mac" {
  ...
  ami           = var.machines[count.index].use_intel ? var.aws_ami_id_intel : var.aws_ami_id_arm
